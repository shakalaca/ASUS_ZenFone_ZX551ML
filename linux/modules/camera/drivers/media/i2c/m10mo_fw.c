...
int Laser_Forcus_sysfs_read_offset(void)
{
	...
	if (IS_ERR_OR_NULL(fp)) {
		pr_err("[isp_nvram] Offset Calibration file open (%s) fail\n", LASERFOCUS_SENSOR_OFFSET_CALIBRATION_FILE);
		return -ENOENT;	/*No such file or directory*/
	}
	...
	if (fp->f_op != NULL && fp->f_op->read != NULL) {
		...
		pr_err("[isp_nvram] Offset Calibration file strlen f_op=NULL or op->read=NULL\n");
		return -ENXIO;	/*No such device or address*/
	}
	set_fs(old_fs);
	filp_close(fp, NULL);
	...
	if(cal_val < 0) {
		pr_err("[isp_nvram] Offset Calibration file is NEGATIVE. (%d)\n", cal_val);
		return -EINVAL;	/*Invalid argument*/
	} else {
		printk("[isp_nvram] Read Offset Calibration value : %d\n", cal_val);
	}
...
int Laser_Forcus_sysfs_read_cross_talk_offset(void)
{
	...
	if (fp->f_op != NULL && fp->f_op->read != NULL) {
		...
		pr_err("[isp_nvram] Cross-talk Offset Calibration file strlen f_op=NULL or op->read=NULL\n");
		return -ENXIO;	/*No such device or address*/
	}
	set_fs(old_fs);
	filp_close(fp, NULL);
	...
	sscanf(buf, "%d", &cal_val);
	if(cal_val < 0) {
		pr_err("[isp_nvram] Cross-talk Offset Calibration file is NEGATIVE. (%d)\n", cal_val);
		return -EINVAL;	/*Invalid argument*/
	} else {
		printk("[isp_nvram] Read Cross-talk Offset Calibration value : %d\n", cal_val);
	}
...
int checkIspNvram(struct m10mo_device *m10mo_dev){
	int val;
	u32 ls_file_reg24, ls_isp_reg24;
	u32 ls_file_reg1e, ls_isp_reg1e;

	/* Read Calibration data */
    printk("[isp_nvram]start\n");
	ls_file_reg1e = Laser_Forcus_sysfs_read_cross_talk_offset();
    if ((int)ls_file_reg1e < 0) goto error;

	ls_file_reg24 = Laser_Forcus_sysfs_read_offset();
     if ((int)ls_file_reg24 < 0) goto error;

    printk("[isp_nvram]file %d, %d\n", ls_file_reg1e, ls_file_reg24);

    (void) m10mo_writew(&m10mo_dev->sd,0x0D,0xA7,246); //laser_sensor_reg1e_calibration
    (void) m10mo_writeb(&m10mo_dev->sd,0x0D,0xA6,0x02); // 2byte read
    (void) m10mo_writeb(&m10mo_dev->sd,0x0D,0xA5,0x01);
    msleep(50);

    (void) m10mo_readb(&m10mo_dev->sd,0x0D,0xAE,&val);
    if (0x00 == val)
    {
        (void) m10mo_readw(&m10mo_dev->sd,0x0D,0xA9,&ls_isp_reg1e);
        printk("[isp_nvram]NVM_Reg1E read:0x%x\n", ls_isp_reg1e);
    }
	else
	{
        goto error;
   	...
   	(void)  m10mo_writew(&m10mo_dev->sd,0x0D,0xA7,245); //laser_sensor_reg24_calibration
    (void)  m10mo_writeb(&m10mo_dev->sd,0x0D,0xA6,0x01); // 1byte read
    (void)  m10mo_writeb(&m10mo_dev->sd,0x0D,0xA5,0x01);
    msleep(50);
    ...
    if (0x00 == val)
    {
        (void)  m10mo_readb(&m10mo_dev->sd,0x0D,0xA9,&ls_isp_reg24);
        printk("[isp_nvram]NVM_Reg24 read:0x%x\n", ls_isp_reg24);
    }
	else
	{
        goto error;
    ...
    if ( ls_isp_reg1e != ls_file_reg1e)
    {
            (void)  m10mo_writew(&m10mo_dev->sd,0x0D,0xA7,246); //laser_sensor_reg1e_calibration
            (void)  m10mo_writew(&m10mo_dev->sd,0x0D,0xA9,ls_file_reg1e);
            (void)  m10mo_writeb(&m10mo_dev->sd,0x0D,0xA6,0x0A); // 2byte write
            (void)  m10mo_writeb(&m10mo_dev->sd,0x0D,0xA5,0x03);
            msleep(50);

            (void)  m10mo_readb(&m10mo_dev->sd,0x0D,0xAE,&val);
            if (0x00 == val)
            {
                printk("[isp_nvram]NVM_Reg1E done:0x%x\n", ls_file_reg1e);
            }
            else
            {
                goto error;
    ...
    if ( ls_isp_reg24 != ls_file_reg24)
    {
        (void)  m10mo_writew(&m10mo_dev->sd,0x0D,0xA7,245); //laser_sensor_reg24_calibration
        (void)  m10mo_writeb(&m10mo_dev->sd,0x0D,0xA9,ls_file_reg24);
        (void)  m10mo_writeb(&m10mo_dev->sd,0x0D,0xA6,0x09); // 1byte write
        (void)  m10mo_writeb(&m10mo_dev->sd,0x0D,0xA5,0x03);
        msleep(50);
        ...
        if (0x00 == val)
        {
            printk("[isp_nvram]NVM_Reg24 done:0x%x\n", ls_file_reg24);
        }
        else
        {
            goto error;
...
error:
	printk("[isp_nvram] done\n");
	return 0;
}
EXPORT_SYMBOL_GPL(checkIspNvram);
